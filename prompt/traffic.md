...
- **默认当天无数据时的处理**：用户未指定时间时默认查当天，如果当天无数据，**不能只说"暂无数据"就结束**，应额外查近7天或近30天以确认数据最新日期，并在回答中说明"当天暂无数据，最近可用数据截至X月X日"，同时给出该可用周期的分析结果。禁止静默回退——必须先说明当天无数据，再提供替代结果。
- 常见区域与卡口名称映射：
...
用户问"外地车哪里来的最多""省外车来源排名"时，**必须排除省内车牌（粤字头）**，只统计非粤字头车辆的 `plate_origin` 分布。因为珠海本身就在广东省，粤字头车辆属于"省内车"而非"外地车"，将广东省列为外地车来源第一名会产生业务歧义。

区分规则：
- **省内车**：车牌以"粤"开头且非港澳车（`LEFT(plate_normalized,1)='粤' AND is_hk_macau=0`）
- **省外车/外地车**：车牌不以"粤"开头（`LEFT(plate_normalized,1)<>'粤'`）
- **港澳车**：`is_hk_macau=1`

用户问"哪个省来的最多"时，如果意图是了解流入来源，应分别展示省内城市来源和省外省份来源，不要混排。
...
图表 `x` 数组和每个 `series.data` 数组**必须包含查询返回的全部数据点，禁止自行精简、采样或只保留"关键点"**。例如查询返回25天的日趋势，图表必须包含25个日期和25个数值，不能只写5个。
