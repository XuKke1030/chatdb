...
| 5 | 问数/人流 | 人流进出趋势、节假日对比 | 部分完成 | `fastPopWeekTrendQuery`、`fastPopHolidayCompareQuery` | 无正式人流表结构；节假日对比当前是简化环比，不满足“上一年同期 + 节假日前” |
| 6 | 问数/人流 | 流动人口识别 | 未完成 | 未发现正式模型或接口 | 需 AIDGP 提供户籍地、运营商驻留地、统计口径或脱敏聚合接口 |
| 7 | 问数/车流 | 车流数据处理与预聚合 | 部分完成 | `traffic_gate_record`、`Aggregate`、快路径统计 | 仅实时聚合，无预聚合表、驻留时长、节假日维度、同比指标 |
...
| 11 | 问数/结果 | 模糊问题澄清 | 未完成 | Prompt 有原则，未发现标准化澄清选项事件 | 需后端返回 `clarification` 结构和前端按钮交互 |
| 12 | 问数/结果 | 三级结构回答、智能图表 | 部分完成 | `prompt/main.md`、快路径 `formatFastPathAnswer`、`chatdb-chart` | 大模型路径依赖 Prompt，不保证稳定；图表切换为前端能力 |
...
| 17 | 问数/支撑 | AI 聚合与指标库 | 部分完成 | 快路径聚合、MCP SQL 查询 | 没有指标库模型、指标沉淀、批计算任务 |
...
| 39 | 问答/对话 | 对话重置 | 已完成 | `/qa/sessions/{sessionId}/reset` | 前端按钮需验收 |
...
说明：首次在沙箱内运行因 Go 构建缓存目录权限失败，提权后测试通过。
